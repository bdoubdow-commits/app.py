...
# --- 1. 画数・数値変換データ ---
# ひらがなとアルファベットはあえて「画数」というノイズとして残す
alphabet_strokes = {
...
def get_numeric_value(text, stroke_dict):
    """
    文字列を数値化する。辞書にある文字は画数、ない文字（漢字）はUnicodeを文字列として繋げる。
    """
    s = ""
    for char in text:
        ...
        if char_upper in stroke_dict:
            ...
            # 辞書にない漢字などはUnicode（ord）をそのまま数値文字列として採用
            s += str(ord(char))
...
st.markdown("""
    ...
    .resonance-title { color: #d4af37; font-size: 0.85em; letter-spacing: 0.15em; text-transform: uppercase; margin-bottom: 6px; }
...
with st.container():
    ...
    with col1:
        last_name = st.text_input("姓（漢字・ひらがな）", placeholder="例：小森")
        first_name = st.text_input("名（漢字・ひらがな）", placeholder="例：葵")
    with col2:
        last_name_alpha = st.text_input("姓（ローマ字）", placeholder="例：KOMORI")
        first_name_alpha = st.text_input("名（ローマ字）", placeholder="例：AOI")
    
    dob = st.date_input("生年月日", min_value=datetime.date(1900, 1, 1), value=datetime.date(1983, 12, 26))

    st.subheader("【追加情報（デバフ判定用）】")
    col3, col4 = st.columns(2)
    with col3:
        time_unknown = st.checkbox("出生時間が不明")
        tob = st.time_input("出生時間", value=datetime.time(14, 30), disabled=time_unknown)
    with col4:
        blood_type = st.selectbox("血液型", ["A型", "B型", "O型", "AB型", "不明（ペナルティ）"], index=4)

    predict_button = st.button("運命を解析する（ハッシュ演算）")
...
if predict_button:
    ...
    # --- Step 2: 基礎スコアの算出（MAX 5.0） ---
    scores = {str(i): min(float(counts.get(str(i), 0)), 5.0) for i in range(10)}
    ...
    if resonance_pct >= 75: resonance_label, resonance_color = "極めて強い共鳴 ✨", "#d4af37"
    elif resonance_pct >= 50: resonance_label, resonance_color = "良好な共鳴 🌙", "#a0c4ff"
    elif resonance_pct >= 25: resonance_label, resonance_color = "微弱な共鳴 🌫️", "#888"
    else: resonance_label, resonance_color = "宿命との乖離 🌑", "#c04040"
    ...
    values = [final_scores[str(i)] for i in range(10)]
    fig = go.Figure()
    fig.add_trace(go.Scatterpolar(r=values+[values[0]], theta=labels+[labels[0]], fill='toself', 
                                 fillcolor='rgba(212, 175, 55, 0.4)', line=dict(color='#d4af37', width=2)))
    fig.update_layout(polar=dict(radialaxis=dict(visible=True, range=[0, 5])), showlegend=False, 
                      paper_bgcolor="rgba(0,0,0,0)", plot_bgcolor="rgba(0,0,0,0)")
    st.plotly_chart(fig, use_container_width=True)

    # --- Step 7: テキスト出力 ---
    st.write(f"【裏演算データ】:`{hash_str}` | 共鳴係数: `{resonance:.4f}`")
    max_val = max(final_scores.values())
    if max_val > 0:
        top_traits = [fortune_map[k] for k, v in final_scores.items() if v == max_val]
        st.info(f"あなたの今日の武器は **【{' / '.join(top_traits)}】** です。（スコア: {max_val:.2f}）")
