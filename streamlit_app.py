...
# --- ページ設定 ---
st.set_page_config(page_title="RESONANCE - 絶対宿宿命律 -", page_icon="🔮", layout="centered")
...
trait_details = {
    ...
    '1': {
        ...
        'base_low': '【本質】決断の淵に立たされた時、他者の波動に共鳴しすぎる傾向があります。<br>【深掘り】自らの真なる声よりも、周囲との調和や協調性を優先してしまうため、勝負所で身を引いてしまうことが多いでしょう。',
    ...
    '6': {
        ...
        'today_high': '【波動解析】愛と調和のエネルギーが満ち溢れ、結びつきが最強となる日です。<br>【深掘り】魂を許し合える者との時間が、あなたに<span style="color:#d4af37">無尽蔵の力</span>を与えます。外での闘争は今日はいったん忘れなさい。<br>【本日の指針】大切な者との対話に時間を使いなさい。',
    ...
    '9': {
        ...
        'today_high': '【波動解析】カリスマの波動が限界突破し、周囲の空間を完全に支配する日です。<br>【深掘り】誰の顔色を窺う必要もありません。あなたの意志が法となり、一挙手一投足が時代の基準となります。すべては思いのままに。<br>【本日の指針】圧倒的な自信を纏い、<span style="color:#d4af37">王として</span>世界に君臨しなさい。',
...
with st.container():
    ...
    # min_valueとmax_valueを明示的に指定して範囲を解放
    dob = st.date_input(
        "生誕の日", 
    ...
    with col3:
        ...
        tob = st.time_input("生誕時刻", value=datetime.time(12, 0), disabled=time_unknown)
    with col4:
...
def get_numeric_value(text, stroke_dict):
    s = "".join([stroke_dict.get(c.upper(), str(ord(c))) for c in text])
...
if predict_button:
    ...
    v_base = f"{abs(year-md)}{abs(get_numeric_value(last_name, hiragana_strokes)-get_numeric_value(first_name, hiragana_strokes))}"
    ...
    sc_base = calc_scores(v_base, "b")
    sc_today = calc_scores(v_base + now.strftime('%Y%m%d%H%M'), "t")

    # グラフ表示
    labels = [fortune_map[str(i)] for i in range(10)]
    fig = go.Figure()
    fig.add_trace(go.Scatterpolar(r=[sc_base[str(i)] for i in range(10)] + [sc_base['0']], theta=labels + [labels[0]], fill='none', line=dict(color='#555', dash='dot'), name='本来の宿命'))
    fig.add_trace(go.Scatterpolar(r=[sc_today[str(i)] for i in range(10)] + [sc_today['0']], theta=labels + [labels[0]], fill='toself', fillcolor='rgba(212,175,55,0.2)', line=dict(color='#d4af37', width=3), name='本日の運勢'))
